...
namespace Prostoi.Models.Repositories
{
  ...
  public class CCMRepository : ICCMRepository
  {
    ...
    public async Task<IEnumerable<LostIdle>> GetMNLZ5LostIdles(string begin, string end)
    {
      var stmt = @"
        ...
        SELECT distinct
          ...
          L2PRD.REP_CCM_PRODUCT_ORDERS.PROFILE_ID as profile,
      ...
      try
      {
        ...
        {
          var lostIdle = createLostIdle(reader);
          result.Add(lostIdle);
        }
        ...
        while (await reader.ReadAsync())
          complementLostIdle(reader, result);
    ...
    public async Task<IEnumerable<LostIdle>> GetMNLZ2LostIdles(string begin, string end)
    {
      var stmt = @"
        ...
        SELECT distinct
          ...
          outtr.steel_grade_id as mark,
          outtr.practice_id as pId, 										
          MS_MECHEL_ESPC6_SC2.REP_CCM_PRODUCT_ORDERS.WIDTH as width,						
          MS_MECHEL_ESPC6_SC2.REP_CCM_PRODUCT_ORDERS.THICKNESS as thickness,	
          MS_MECHEL_ESPC6_SC2.REP_CCM_PRODUCT_ORDERS.PROFILE_ID as profile,
          DELAYS.timeinterval as realinterval,
          CASE WHEN DELAYS.timeinterval < 0 THEN 0 ELSE ROUND(DELAYS.timeinterval * 5, 0) END as count
        FROM ( 
          SELECT 
          ...
          FROM 
            ...
            TRUNC(STOP_DATE + INTERVAL '4' hour + INTERVAL '30' minute, 'DDD') as STOP_DATE,
            report_counter
            FROM MS_MECHEL_ESPC6_SC2.REPORTS, p
            WHERE 
            STOP_DATE + INTERVAL '4' hour + INTERVAL '30' minute >= p.startT
            and STOP_DATE + INTERVAL '4' hour + INTERVAL '30' minute < p.stopT
          )
        ...
        JOIN MS_MECHEL_ESPC6_SC2.CCM_GENERAL_PROGRAM_VARS
          ON MS_MECHEL_ESPC6_SC2.CCM_GENERAL_PROGRAM_VARS.PRACTICE_ID = outtr.practice_id 
          AND MS_MECHEL_ESPC6_SC2.CCM_GENERAL_PROGRAM_VARS.VAR_CODE = 52003 
          AND AREA_ID = 1100

        JOIN MS_MECHEL_ESPC6_SC2.REP_CCM_PRODUCT_ORDERS
          ON outtr.REPORT_COUNTER = REP_CCM_PRODUCT_ORDERS.REPORT_COUNTER
        ...
        JOIN (
          ...
          FROM (
            ...
            FROM
              (
            SELECT delay_start_time,
              COALESCE(NULLIF(delay_stop_time, TO_DATE('01-01-1970', 'DD-MM-YYYY')), SYSDATE) as
        delay_stop_time,
              delay_start_time + INTERVAL '4' hour + INTERVAL '30' minute as delay_start_shift,
              COALESCE(NULLIF(delay_stop_time, TO_DATE('01-01-1970', 'DD-MM-YYYY')), SYSDATE) + INTERVAL '4' hour + INTERVAL '30' minute as delay_stop_shift
            FROM MS_MECHEL_ESPC6_SC2.REP_DELAYS, p
            WHERE 
              (DELAY_START_TIME + INTERVAL '4' hour + INTERVAL '30' minute >= p.startT
              AND DELAY_START_TIME + INTERVAL '4' hour + INTERVAL '30' minute < p.stopT)
                  OR
      ...
      try
      {
        await _db2.OpenAsync();

        var cmd = new OracleCommand(stmt, _db2);
        cmd.Parameters.Add("bDate", begin);
        cmd.Parameters.Add("eDate", end);
        ...
        cmd.CommandText = @"
          ...
          JOIN MS_MECHEL_ESPC6_SC2.CCM_GENERAL_PROGRAM_VARS
          ON MS_MECHEL_ESPC6_SC2.CCM_GENERAL_PROGRAM_VARS.PRACTICE_ID = outtr.practice_id AND MS_MECHEL_ESPC6_SC2.CCM_GENERAL_PROGRAM_VARS.VAR_CODE = 52003 AND AREA_ID = 1100
          JOIN MS_MECHEL_ESPC6_SC2.REP_CCM_PRODUCT_ORDERS
          ON outtr.REPORT_COUNTER = REP_CCM_PRODUCT_ORDERS.REPORT_COUNTER
          JOIN (
            SELECT delayDate,
              ...
              FROM (
                ...
                FROM (
                  SELECT (length_at_end - length_at_beginning) * (  -0.01 *percentage_of_deviation / (1-0.01 * percentage_of_deviation)) as Undercast_length,
                    ...
                    start_downtime + INTERVAL '4' hour + INTERVAL '30' minute as delay_start_shift,
                    end_downtime + INTERVAL '4' hour + INTERVAL '30' minute as delay_stop_shift
                  FROM MS_MECHEL_ESPC6_SC2.REP_CCM_DOWNTIME, p
                  WHERE  (start_downtime + INTERVAL '4' hour + INTERVAL '30' minute >= p.startT
                AND start_downtime + INTERVAL '4' hour + INTERVAL '30' minute < p.stopT)
                    OR
                    (end_downtime + INTERVAL '4' hour + INTERVAL '30' minute >= p.startT
    ...
    private void complementLostIdle(System.Data.Common.DbDataReader reader, List<LostIdle> idleList)
    {
      ...
      LostIdle idle = idleList.Find(li => li.Day == date);
      if (idle != null)
      {
